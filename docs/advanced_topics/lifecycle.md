...
Constructor (`MyImportRunner`) of your `IImportRunner` is called. You should provide the values set by the configuration items. Read [User configuration - storage]({% link docs/advanced_topics/user_configuration.md %}) for information about storage usage.
